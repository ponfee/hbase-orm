...
public class BeanEntityDaoTest extends SpringBaseTest<BeanEntityDao> {

    @Repository("beanEntityDao")
    public static class BeanEntityDao extends HbaseBatchDao<BeanEntity, String> {
    }
    ...
    public void batchPut() {
        List<BeanEntity> batch = new ArrayList<>();
        for (int i = 0; i < 1; i++) {
    ...
    @Test
    public void findAll() {
        List<BeanEntity> list = (List<BeanEntity>) getBean().range(null, null);
    ...
    public void previousPageAll() {
        ...
        List<BeanEntity> data = new ArrayList<>();
        int count = 1;
        List<BeanEntity> list = (List<BeanEntity>) getBean().previousPage(query);
        while (CollectionUtils.isNotEmpty(list) && list.size() == query.pageSize()) {
            ...
            list = (List<BeanEntity>) getBean().previousPage(query);
        }
